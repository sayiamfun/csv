...
import com.sayiamfun.common.utils.UnZipFile;
...
//        lostMon = Arrays.asList("".split(","));
     ...
     * @throws IOException
     */
    private static void unZipPath(String inputPath, String inputPath1) throws IOException {
        Set<String> strings = ScanPackage.scanThisDirectoryFile(inputPath);
        strings.addAll(ScanPackage.scanThisDirectoryFile(inputPath1));
        ...
        for (String string : strings) {
            for (String vin : vins) {
                if (string.contains(vin.trim())) {
                    UnZipFile.unZipFiles(string, "/Volumes/UsbDisk/veh_49_originData/");
                    break;
                }
            }
        }
